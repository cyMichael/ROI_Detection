...
from utils.utils import print_network, get_optim, get_split_loader, calculate_error
import os
...
def summary_auc(model, loader, n_classes):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    acc_logger = Accuracy_Logger(n_classes=n_classes)
    model.eval()
    test_error = 0.
    ...
    print("test_error: {:.4f}".format(test_error))
    print("auc: {:.4f}".format(auc))
    return patient_results, test_error, auc, all_probs
